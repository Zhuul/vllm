...
def apply_git_patch(patch_file: Path, overlay_mode: bool = False, track_file: Path = None):
    try:
        # Check if patch applies cleanly
        subprocess.run(['git', 'apply', '--check', str(patch_file)], check=True, capture_output=True)
        # Apply the patch
        subprocess.run(['git', 'apply', str(patch_file)], check=True, capture_output=True)
        ...
        try:
            subprocess.run(['git', 'apply', '--reverse', '--check', str(patch_file)], check=True, capture_output=True)
...
def patch_fallback(name: str):
    ...
    if name == "qutlass-fetchcontent-base":
        ...
        if not qutlass_file.exists(): return False
        with open(qutlass_file, 'r', encoding='utf-8') as f:
            content = f.read()

        if '_qutlass_fetch_base' in content:
            print("[patches] qutlass-fetchcontent-base is already applied via fallback.")
            return True

        # Insert variables at top
        new_content = content.replace('set(CUTLASS_INCLUDE_DIR "${CUTLASS_INCLUDE_DIR}" CACHE PATH "Path to CUTLASS include/ directory")',
            ...
            'set(_qutlass_default_binary_dir "${_qutlass_fetch_base}/qutlass-build")\n'
        ...
        new_content = new_content.replace('GIT_PROGRESS TRUE\n    CONFIGURE_COMMAND',
            'GIT_PROGRESS TRUE\n'
        ...
        new_content = new_content.replace('message(STATUS "[QUTLASS] QuTLASS is available at ${qutlass_SOURCE_DIR}")\n',
        ...
        if cmakelists_file.exists():
            ...
            if 'VLLM_ENABLED_OPTIONAL_CUDA_LIBS' not in c_content:
                c_new = c_content.replace('include(${CMAKE_CURRENT_LIST_DIR}/cmake/utils.cmake)',
                ...
                print("[patches] fallback for cuda-optional-flags (CMakeLists.txt) already applied.")
                applied = True
        ...
        if setup_file.exists():
            ...
            if 'cuda_optional_envs' not in s_content:
                s_new = s_content.replace('cmake_args += [f"-DCMAKE_CUDA_COMPILER={CUDA_HOME}/bin/nvcc"]',
                ...
                if s_new != s_content:
                 ...
                 print("[patches] fallback for cuda-optional-flags (setup.py) already applied.")
                 applied = True
...
def main():
    ...
    parser.add_argument('--config', type=str, default='extras/patches/patches.json', help="Path to patches configuration file.")
    parser.add_argument('--overlay-mode', action='store_true', help="Enable overlay mode (for containers).")
    parser.add_argument('--track-file', type=str, default='/opt/work/tmp/vllm_patched_files.txt', help="File to track patched files in overlay mode.")
    parser.add_argument('--reset', action='store_true', help="Reset patched files based on track file.")
    ...
    overlay_mode = args.overlay_mode or os.environ.get('PYTHON_PATCH_OVERLAY', '0') == '1'
    ...
    print(f"[patches] Applying {len(config['patches'])} patches (Overlay Mode: {overlay_mode})")
    ...
    for patch_info in config['patches']:
        ...
        if overlay_mode and patch_info.get('skip_in_overlay', False) and diff_file.name == '0001-cumem-alloc-env-fallback.diff':
            print(f"[patches] Skipping {name} in overlay mode.")
            continue

        success = apply_git_patch(diff_file, overlay_mode=overlay_mode, track_file=track_file)

        if not success:
            if 'fallback' in patch_info and patch_info['fallback']['type'] == 'regex_replace':
                print(f"[patches] Attempting configured fallback for {name}...")
                fallback = patch_info['fallback']
                target_file = Path(fallback['file'])
                fallback_regex_replace(target_file, fallback['pattern'], fallback['replacement'])
            else:
                patch_fallback(name)
